...
const FinanceView: React.FC<FinanceViewProps> = ({ accounts, accountBalances, transactions, budgets, onSaveAccount, onSaveTransaction, onSaveBudget }) => {
  ...
  const handleExportExcel = () => {
    const exportData = accounts.map(acc => {
        const accBalance = accountBalances.get(acc.id) || 0;
        ...
        return {
            'اسم الحساب': acc.name,
            'الرصيد الحالي': accBalance.toFixed(2),
            'إجمالي المقبوضات (+)': totalIn.toFixed(2),
            'إجمالي المدفوعات (-)': totalOut.toFixed(2),
            'نوع الحساب': acc.type === 'cash' ? 'نقد' : acc.type === 'bank' ? 'بنك' : 'آخر'
        };
    });
    exportToExcel(exportData, 'تقرير_الخزينة_والحسابات', 'Treasury', 'ملخص أرصدة وحركات الحسابات المالية');
  };
        ...
        <div className="lg:col-span-2 space-y-6">
            <div className="bg-white shadow-lg rounded-xl p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-bold text-slate-800">أرصدة الحسابات</h3>
                    <button 

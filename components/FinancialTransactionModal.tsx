...
type ModalTransactionType = Extract<FinancialTransactionType, 'expense' | 'capital_deposit' | 'transfer'>;
...
const FinancialTransactionModal: React.FC<FinancialTransactionModalProps> = ({ type, accounts, budgets, onClose, onSave }) => {
  ...
  const { title, fields } = useMemo(() => {
    switch (type) {
      ...
      case 'transfer':
        return { title: 'تحويل بين الحسابات', fields: ['amount', 'from', 'to', 'description', 'category'] };
      default:
    ...
    <Modal isOpen={true} onClose={onClose} title={title} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {fields.includes('category') && (
          <div>
            <label htmlFor="category-input" className="block text-slate-700 text-sm font-bold mb-2">التصنيف / الغرض</label>
